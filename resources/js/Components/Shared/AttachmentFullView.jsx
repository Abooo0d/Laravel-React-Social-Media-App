...
const AttachmentFullView = ({ attachments, index, show, setShow }) => {
  ...
  const next = (index) => {
    if (index < message?.attachments?.length - 1) {
      setAttachmentIndex(index + 1);
      setAttachmentId(message?.attachments[index + 1]);
    } else {
      setAttachmentIndex(0);
      setAttachmentId(message?.attachments[0] ?? 0);
    }
  };
  const back = (index) => {
    if (index > 0) {
      ...
      setAttachmentIndex(message?.attachments?.length - 1);
      setAttachmentId(message?.attachments[message?.attachments?.length - 1]);
    }
  };
  return (
    <div
          ...
          <div className="relative h-full flex justify-center items-center flex-1 max-w-[80%]">
            <div
              className={`relative rounded-md overflow-hidden flex justify-start items-enter cursor-pointer duration-200 border-transparent border-[1px] border-solid hover:border-gray-500 hover:scale-105 group
                      ${
                        MessageIsAudio(message?.attachments[attachmentIndex])
                          ? "w-80 h-20 max-w-[70%]"
                          : MessageIsVideo(
                              message?.attachments[attachmentIndex]
                            )
                          ? "w-80 h-80 max-w-[70%]"
                          : MessageIsPDF(message?.attachments[attachmentIndex])
            ...
            >
              {MessageIsImage(message?.attachments[attachmentIndex]) && (
                <>
                  <img
                    src={message?.attachments[attachmentIndex]?.url}
                    alt="attachment"
                    className="min-w-20 min-h-20 rounded-md object-cover"
                  />
                </>
              )}
              {MessageIsVideo(message?.attachments[attachmentIndex]) && (
                <CustomVideoPlayer
              ...
              {!isPreviewAble(message?.attachments[attachmentIndex]) && (
                <>
                  ...
                  <div className="w-full h-full  flex justify-between items-center gap-2 p-2">
                    ...
                    <h3 className="flex-1 text-[15px] text-gray-400 break-all">
                      {message?.attachments[attachmentIndex]?.name}
                    </h3>
                  </div>
                </>
              )}
