...
const MessageCard = ({ message }) => {
  ...
  return (
    ...
    >
      ...
      >
        <div
          className={`backdrop-blur-sm relative w-fit p-2 rounded-md dark:text-gray-300 text-gray-600 word-wrap cursor-default max-w-[80%] ove flex justify-center items-start flex-col break-all  ${
            message?.user.id != user.id
              ? "dark:bg-[#1f2937] bg-gray-400"
              : "dark:bg-[#005A9C] pr-8 bg-gray-300"
          }`}
        >
          ...
          <div
            className={`absolute bottom-0 ${
              message?.user.id != user.id
                ? "left-[-10px] w-[20px] h-[20px] border-[10px] border-solid border-transparent dark:border-b-[#1f2937] border-b-gray-400 z-10"
                : "right-[-10px] w-[20px] h-[20px] border-[10px] border-solid border-transparent dark:border-b-[#005A9C] border-b-gray-300 z-10"
            }`}
          />
        </div>
        <img
